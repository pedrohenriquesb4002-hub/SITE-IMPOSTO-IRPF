...
export const meioEnvioEnum = pgEnum("meioEnvio", [
  "WhatsApp",
  "Email",
  "SMS",
  "Presencial",
...
export const declarations = pgTable(
  "declarations",
  {
    ...
    cpfCliente: varchar("cpfCliente", { length: 30 }),
  ...
  (table) => ({
    ...
    idxUserCollaborator: index("idx_user_collaborator").on(
      table.userId,
      table.collaborator
    ),
    idxUserStatus: index("idx_user_status").on(
      table.userId,
...
export const itrDeclarations = pgTable(
  "itrDeclarations",
  {
    id: serial("id").primaryKey(),
    userId: integer("userId")
  ...
  (table) => ({
    ...
    idxItrUserCollaborator: index("idx_itr_user_collaborator").on(
      table.userId,
      table.collaborator
    ),
    idxItrUserStatus: index("idx_itr_user_status").on(
      table.userId,
      table.statusPagamento
    ),
  })
...
export const collaborators = pgTable(
  ...
  (table) => ({
    uniqueUserCollaborator: unique("uq_collaborator_user_name").on(
      table.userId,
      table.name
    ),
  })
...
export const quotas = pgTable(
  ...
  (table) => ({
    idxQuotasUserCollaborator: index("idx_quotas_user_collaborator").on(
      table.userId,
      table.collaborator
    ),
  })
...
export const quotaTracking = pgTable(
  "quotaTracking",
  {
    id: serial("id").primaryKey(),
...
export type QuotaTracking = typeof quotaTracking.$inferSelect;
export type InsertQuotaTracking = typeof quotaTracking.$inferInsert;