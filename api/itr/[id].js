...
export default async function handler(req, res) {
  ...
  try {
    const [decl] = await sql`SELECT id FROM "itrDeclarations" WHERE id = ${id} AND "userId" = ${userId} LIMIT 1`
    if (!decl) return res.status(404).json({ error: 'Declaração ITR não encontrada' })

    if (req.method === 'PUT') {
      const { collaborator, cpfCliente, cliente, valorRecebido, clienteType, statusPagamento } = req.body
      const [current] = await sql`SELECT * FROM "itrDeclarations" WHERE id = ${id} LIMIT 1`
